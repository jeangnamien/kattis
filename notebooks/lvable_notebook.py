...
@app.cell
def __(mo):
    mo.md("# LV-able Problem")
    return


@app.cell
def __(mo):
    text_input = mo.ui.text(value="lovely", label="Text to check")
    return text_input,
...
def __(mo, result, text_input):
    status = "✅ LV-able" if result == "LV" else "❌ Not LV-able"
    mo.md(f"**{text_input.value}** is {status} ({result})")
    return status,
